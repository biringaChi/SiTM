...
from sitm_core.utils.functions import strip_c_comments

"""
Author: Chidera Biringa
"""

cache_file = Path(".sitm/cache.json")
...
def should_rescan(file_path: str, current_hashes: dict, cache_data: dict):
    ...
    unchanged_vuln_lines = all(i not in changed_lines for i in vulnerable_lines.keys())
    if unchanged_vuln_lines and changed_lines:
        return False, []
    return True, changed_lines

def hash_c_function(body: str) -> str:
